...
# ╔═╡ ecb5d735-7796-4f16-a0e1-3f8357e1870c
begin 
	struct O2state{T}
		p::Vector{T}
		v::Vector{T}
	...
	function update!(s::O2state,t;f=t->0,ωₙ=1,ζ=0)
		p₀,v₀,t₀ = s.p[end],s.v[end],s.t[end]
		a = f(t)-ωₙ^2*p₀-2ζ*ωₙ*v₀
		v = v₀+(t-t₀)*a
		p = p₀+(t-t₀)*v
		push!(s,[p,v,t])
	end
...
# ╔═╡ a28b5f10-b362-452d-96b7-f4d42542eb3d
begin 
	struct SS2{T}
		q::Vector{T}
		q̇::Vector{T}
		t::Vector{T}
		function SS2(init::Vector{T}) where T
			q,q̇,t = init
			new{T}([q],[q̇],[t])
		end
	end

	function update!(s::SS2,t;f=t->0,ωₙ=1,ζ=0)
		q₀,q̇₀,t₀ = s.q[end],s.q̇[end],s.t[end]
		Δt = t-t₀; τ = 0.5ωₙ*Δt
		
		q̈₀ = f(t₀)-ωₙ^2*q₀-2ζ*ωₙ*q̇₀
		# q̈ = f(t)-ωₙ^2*q-2ζ*ωₙ*q̇ #<-substitute
		q̈ = f(t)-ωₙ^2*(q₀+0.5Δt*(2q̇₀+0.5Δt*q̈₀))-2ζ*ωₙ*(q̇₀+0.5Δt*q̈₀)
		q̈ /= 1+2ζ*τ+τ^2
		q̇ = q̇₀+0.5Δt*(q̈₀+q̈)	
		q = q₀+0.5Δt*(q̇₀+q̇)

		push!(s.q,q);push!(s.q̇,q̇);push!(s.t,t)
		return s
	end
end
...
begin
	...
	for t in range(0,20π,120)
		...
		update!(A,t,ωₙ=1+0.2*cos(2t),ζ=0.2abs(A.v[end]))
	end
end
...
# ╔═╡ d363d8c6-3b5a-4b79-8d11-69dd14ae1706
begin
	using Plots
	plot(B.t,B.q)
	plot!(A.t,A.p)
	scatter!([62.1],[-0.58],markershape=:cross)
end
