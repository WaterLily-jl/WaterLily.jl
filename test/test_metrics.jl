...
@testset "Metrics.jl" begin
    ...
    for f ∈ arrays
        ...
        @test all(WaterLily.viscous_force(u₃,1.0,df₃,body) .≈ 0)
        # flux force
        @test all(WaterLily.flux_force(u₂,df₂,body) .≈ 0)
        @test all(WaterLily.flux_force(u₃,df₃,body) .≈ 0)
        # viscous moment
        u₂ .= 0; u₃ .= 0
        ...
        # flux moment
        @test all(WaterLily.flux_moment(SVector{2,Float64}(N/2,N/2),u₂,df₂,body) .≈ 0)
        @test all(WaterLily.flux_moment(SVector{3,Float64}(N/2,N/2,N/2),u₃,df₃,body) .≈ 0)
        # pressure moment
