...
#     @test_nowarn rm(test_dir, recursive=true)
# end
# @testset "WaterLilyGeometryBasicsExt.jl" begin
# import some internal functions
split = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).split
split_w = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).split_w
# test
T = Float32
O = SA{T}[0,0,0]
R = SA{T}[3,2,1]
@assert split_w(R,1) == SA{T}[1.5,2,1]
@assert split_w(R,2) == SA{T}[3,1,1]
@assert split_w(R,3) == SA{T}[3,2,1/2]
@btime split_w($R,$3) # 20.302 ns (2 allocations: 64 bytes)
@assert all(split(O,R) .== ((O,SA{T}[1.5,2,1]),(SA{T}[1.5,0,0],SA{T}[1.5,2,1])))
@btime split($O,$R) # 39.738 ns (3 allocations: 128 bytes)
# inside
inside = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).inside
@assert inside(SA{T}[0,0,0],O,R)
@assert !inside(SA{T}[4,0,0],O,R)
x = SA{T}[2,2,1]
@btime inside($x,$O,$R) # 39.738 ns (3 allocations: 128 bytes)
make_bvh = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).make_bvh
bvh = make_bvh(O,R,3,Array)
@assert all(bvh[1] .≈ (O,R))
@assert all(bvh[2] .≈ (SA{T}[0,0,0],SA{T}[1.5,2,1]))
@assert all(bvh[3] .≈ (SA{T}[1.5,0,0], SA{T}[1.5,2,1]))
@assert all(bvh[4] .≈ (SA{T}[0,0,0], SA{T}[1.5,1,1]))
@assert all(bvh[5] .≈ (SA{T}[0,1,0], SA{T}[1.5,1,1]))
@assert all(bvh[6] .≈ (SA{T}[1.5,0,0], SA{T}[1.5,1,1]))
@assert all(bvh[7] .≈ (SA{T}[1.5,1,0], SA{T}[1.5,1,1]))
# helper functions to get indices
leafs = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).leafs
parents = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).parents
children = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).children
@assert all(leafs(3) .== CartesianIndices((4:7,)))
@assert all(parents(3) .== CartesianIndices((3:-1:1,)))
@assert all(children(1) .== 2:3) && all(children(2) .== 4:5) && all(children(3) .== 6:7)
# merge boxes
merge = Base.get_extension(WaterLily, :WaterLilyGeometryBasicsExt).merge
