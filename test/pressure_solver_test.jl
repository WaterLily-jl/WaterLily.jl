...
macro soft_test(ex,tol)
    # get the first argument expression
    args = length(ex.args)==3 ? ex.args : ex.args[2:end]
    # check if we pass the softfail test
    soft = Expr(:call,args[1],:($args[2]),:($tol))
    hard = eval(Expr(:call,args[1],:($args[2]),:($args[3])))
    return quote
        if eval($soft)==true && $hard==false
            @warn "Soft test failed " show($soft)
        else
            @test $ex
        end
    end
...
@testset "Manufactured solution 1" begin
    for psolver in [WaterLily.Poisson,MultiLevelPoisson,HyprePoisson,GeomMultigridPoisson]
...
@testset "hydrostatic pressure" begin
    for psolver in [WaterLily.Poisson,MultiLevelPoisson,HyprePoisson,GeomMultigridPoisson]
        for f ∈ arrays, T in [Float32,Float64], L in [32,64]    #@info "Testing $psolver solver with T=$T on a domain L=$L"
            ...
            # create Poisson solver
            pois = make_poisson((L,L);psolver,T,mem=f)
            MG = isa(pois,MultiLevelPoisson) || psolver==GeomMultigridPoisson
...
@testset "Potential flow" begin
    for psolver in [WaterLily.Poisson,MultiLevelPoisson,GeomMultigridPoisson]
        for f ∈ arrays, T in [Float32,Float64], L in [32,64]    #@info "Testing $psolver solver with T=$T on a domain L=$L"
            # create Poisson solver
            pois = make_poisson((L,L);psolver,T,mem=f)
            MG = isa(pois,MultiLevelPoisson) || psolver==GeomMultigridPoisson
