...
is applied to the tangential components. `saveexit=true` leaves the normal
component on the `x` exit face (upper `x` boundary) untouched, so a convective
outflow set by `exitBC!` is kept; every other face is always overwritten.
`perdir` lists the dimensions using periodic BCs instead. `t` is passed to `U`
when it is a function `(i,x,t)->...`.
"""
